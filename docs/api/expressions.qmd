...
::: {.callout-tip}
## Vector Variables
For creating arrays of variables (e.g., `x[0]`, `x[1]`), use [VectorVariable](vectors.qmd) instead of creating individual `Variable` objects in a loop.
:::

```python
from optyx import Variable

x = Variable(name, lb=None, ub=None, domain="continuous", obj=0.0)
```
...
| `obj` | `float` | Mutable linear objective coefficient added to the problem objective | `0.0` |
...
| `.obj` | `float` | Mutable linear objective coefficient used on the next solve |
...
# Binary variable (enforced by the MILP solver for linear models)
b = Variable("b", domain="binary")
print(f"Binary: lb={b.lb}, ub={b.ub}")
```

Invalid domains, non-finite bounds, or bounds where `lb > ub` raise a
descriptive validation error immediately. Mutating `lb` or `ub` applies the
same validation and leaves the previous value unchanged if validation fails.

For linear models, `obj` can be used alongside the expression passed to
`minimize()` or `maximize()`. Updating it is reflected on the next solve:

```python
x = Variable("x", lb=0, ub=10, obj=1)
problem = Problem().minimize(0).subject_to(x >= 0)
first = problem.solve()
