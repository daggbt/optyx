...
Add one or more constraints to the problem. Accepts a single constraint, a list of constraints, or a generator expression.
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `constraint` | `Constraint \| Iterable[Constraint]` | Constraint(s) to add |
...
### `.subject_to_matrix(A, x, sense, b)`

Add matrix constraints `A @ x {<=, >=, ==} b`, bypassing expression tree construction. Accepts dense NumPy arrays or `scipy.sparse` matrices for efficient large-scale LP.

```python
import scipy.sparse as sp

A = sp.random(1000, 10000, density=0.01, format="csr")
b = np.ones(1000)
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `A` | `np.ndarray \| scipy.sparse` | Coefficient matrix (m × n) |
| `x` | `VectorVariable` | Variable vector of size n |
