...
description: "Mean-variance portfolio optimization with covariance using QuadraticForm"
date: last-modified
---

## Introduction

This tutorial builds a complete **Markowitz mean-variance portfolio optimization** model using Optyx's v1.2.0 features. You'll see how VectorVariable, QuadraticForm, and Parameter work together for realistic portfolio problems.
...
```{python}
from optyx import VectorVariable, Problem, QuadraticForm
...
portfolio_variance = QuadraticForm(weights, covariance)
...
::: {.callout-note}
## QuadraticForm Efficiency
`QuadraticForm(weights, covariance)` computes `w.T @ Σ @ w` with an analytic gradient:
`∇ variance = 2 Σ w`. No loop over n² terms needed.
:::
...
def solve_for_target(target_return):
    ...
    port_var = QuadraticForm(w, covariance)
...
```{python}
from optyx import VectorVariable, Parameter, Problem, QuadraticForm
...
solution = (
    ...
    .minimize(QuadraticForm(w, covariance))
    .subject_to(w.sum().eq(1))
...
sol_minvar = (
    Problem()
    .minimize(QuadraticForm(w_minvar, covariance))
...
| `QuadraticForm` | O(1) gradient for variance |
| `Parameter` | Fast re-solves for scenarios |
| Vector operations | No explicit loops needed |

Key takeaways:

1. **Use QuadraticForm** for portfolio variance—it's much faster than expanding the sum
