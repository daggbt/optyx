...
x = [Variable(f"trucks_{i}", lb=0, ub=n_trucks) for i in range(n_shovels)]

# Throughput from each shovel
throughputs = [
    x[i] * truck_payload * 60 / cycle_times[i] 
    for i in range(n_shovels)
]
total_throughput = sum(throughputs)

# Constraints
fleet_constraint = sum(x) <= n_trucks
crusher_constraint = total_throughput <= crusher_capacity
shovel_constraints = [throughputs[i] <= shovel_dig_rates[i] for i in range(n_shovels)]

# Build problem
prob = Problem("fleet_dispatch").maximize(total_throughput)
prob.subject_to(fleet_constraint)
prob.subject_to(crusher_constraint)
for c in shovel_constraints:
    prob.subject_to(c)

# Add minimum trucks per active shovel
min_trucks = 1
for i in range(n_shovels):
    prob.subject_to(x[i] >= min_trucks)
```
...
fleet_used = sum(solution[f"trucks_{i}"] for i in range(n_shovels))
