...
- How vectorized gradients keep CQP construction and solves close to raw SciPy in the bundled benchmarks
...
| Loop Pattern | Vectorized Equivalent | Benefit |
|--------------|----------------------|---------|
| `sum(c[i]*x[i] for i ...)` | `c @ x` | Compact LP representation and vectorized coefficient handling |
| `sum(x[i]**2 for i ...)` | `x.dot(x)` | Analytic vector gradient path |
| double loop with `Q` | `x.dot(Q @ x)` | Structured quadratic-form evaluation and gradient |
