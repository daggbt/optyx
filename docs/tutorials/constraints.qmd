...
When dealing with massive constraint sets (10,000+), looping over components is slow. Use `subject_to_matrix()` to bypass symbolic algebra entirely:

```python
# Pass scipy.sparse matrices straight to the solver
prob.subject_to_matrix(A_sparse, x, "<=", b_vector)
```
