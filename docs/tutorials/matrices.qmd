...
For portfolio variance (w.T @ Σ @ w), use `QuadraticForm` with a constant covariance matrix:
...
portfolio_variance = QuadraticForm(weights, cov_matrix)
...
::: {.callout-note}
## Efficient Gradients
QuadraticForm computes gradients analytically: `∇(x.T Q x) = 2Qx`.
This is much faster than differentiating the expanded sum.
:::
