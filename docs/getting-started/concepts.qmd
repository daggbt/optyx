...
# Binary variable (0 or 1) - enforced for linear models by the MILP solver
...
Variable names identify solver columns. Reusing a name is allowed only when it
refers to a compatible declaration of the same logical variable; conflicting
bounds, domains, or distinct declarations with the same name are rejected
before solving.
