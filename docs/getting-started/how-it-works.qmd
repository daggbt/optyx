...
Symbolic evaluation adds dispatch and tree-traversal work compared with an
equivalent hand-written numerical function. The cost depends on expression
structure and whether Optyx can use a vectorized compiled path. End-to-end
solver measurements are reported on the versioned [Benchmarks](../benchmarks.qmd)
page.
...
| Approach | Derivative work | Developer Time |
|----------|-----------------|----------------|
| Hand-written | Depends on the implementation | Higher for complex functions |
| Finite differences | Requires additional function evaluations | Low |
| Optyx autodiff | Exact symbolic rules compiled for the model | Low |
...
if model_is_linear:
    solver = "milp" if has_discrete_variables else "linprog"
elif no_general_constraints:
    solver = "L-BFGS-B"
elif constrained_model_is_large_and_sparse:
    solver = "trust-constr"
elif objective_or_constraint_degree_is_above_two_or_non_polynomial:
    solver = "trust-constr"
else:
    solver = "SLSQP"  # Linear/quadratic constrained NLP, with fallback
```
...
|-----------|----------------------|-----|
| First solve | Model-dependent | Tree building + compilation |
| Each f(x) call | Model-dependent | Compiled symbolic evaluation vs direct computation |
| Each ∇f(x) call | Model-dependent | Generated derivative vs hand-written derivative |
| Re-solve | Usually near raw SciPy | Cached structure avoids repeated compilation; solver work still remains |
