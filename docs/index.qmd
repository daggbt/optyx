...
Optyx is young (v1.2.0) and opinionated. It's **not** a replacement for specialized tools:
...
## QuadraticForm: Efficient Covariance

For portfolio variance (w.T @ Σ @ w), use `QuadraticForm` for analytic gradients:

```{python}
from optyx import VectorVariable, QuadraticForm, Problem
import numpy as np
...
# QuadraticForm computes w.T @ cov @ w with analytic gradient
...
::: {.callout-note}
## Why QuadraticForm?
A loop-built quadratic form creates n² expression nodes. `QuadraticForm` uses a single node with analytic gradient: ∇(w.T Q w) = 2Qw. This is much faster for large n.
:::
...
risk = QuadraticForm(weights, cov)
...
- **Native gradient rules** — L2Norm, L1Norm, QuadraticForm compute gradients without loops
