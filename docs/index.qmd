...
Re-solves avoid rebuilding unchanged symbolic and solver structures. The actual
speedup depends on problem size, solver iterations, and what changed between
solves.
:::
...
The symbolic layer adds model-building and compilation work. The bundled
benchmarks show that the ratio to raw SciPy varies by model type and size. The
payoff comes from exact generated derivatives, cached re-solves, and readable
models you can maintain. Consult the versioned benchmark page rather than
assuming one fixed overhead ratio.
:::
...
|----------|-------------------|-------|
| Linear programs | 1.1-1.8x cold | Latest vectorized results from n=50 to n=5,000; warm overhead is 1.0-1.3x |
| Quadratic NLP (first solve) | 2.1-2.6x | Latest measured unconstrained NLP from n=50 to n=5,000 |
| Quadratic NLP (re-solve) | 1.2-1.5x | Cached structure reduces overhead in the latest run |
...
This documentation is largely generated with the assistance of AI and may
contain errors. If you spot an error or a discrepancy between the documentation
and Optyx's behaviour, please [create an issue](https://github.com/optyx-dev/optyx/issues/new).
:::
